...
bool unordered_array_set_in(unordered_array_set *set, int value) {
    return linerSearch_(set->data, set->size, value);
}
...
void unordered_array_set_insert(unordered_array_set *set, int value) {
    ...
    if (!linerSearch_(set->data, set->size, value))
        append_(set->data, &set->size, value);
}

void unordered_array_set_deleteElement(
        unordered_array_set *set, int value) {
    size_t position = linerSearch(set->data, set->size, value);
    if (position >= 0) {
